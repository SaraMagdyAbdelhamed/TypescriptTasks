...
import { RetryHandler } from "./retry-mechanism";
...
/**
 * A single entry stored inside the cache map.
 */
 ...
 */
async function getReliableData(cache: AsyncCache, retryer: RetryHandler): Promise<unknown> {
    return cache.get(
        "profile_data",
        () =>
            retryer.execute(() =>
...
async function exampleBasicCache(): Promise<void> {
    console.log("\n=== Example 1: Basic Cache Hit / Miss ===");
    const cache = new AsyncCache();
    fetchCount = 0;

    // First call  → MISS, fetches data
    const first = await cache.get("user_1", () => mockFetch("/api/users/1"), 5_000);
    console.log(" First result:", first);

    // Second call → HIT, returns cached promise
    const second = await cache.get("user_1", () => mockFetch("/api/users/1"), 5_000);
...
async function exampleCacheWithRetry(): Promise<void> {
    console.log("\n=== Example 2: AsyncCache + RetryHandler ===");
    fetchCount = 0;

    const cache = new AsyncCache();
    const retryer = new RetryHandler({
        maxAttempts: 5,
        delayMs: 100,
        onRetry: (err, attempt) =>
            console.log(`  ⚠ Retry ${attempt}: ${(err as Error).message}`),
    });
...
async function exampleInvalidation(): Promise<void> {
    console.log("\n=== Example 3: Manual Invalidation ===");
    fetchCount = 0;

    const cache = new AsyncCache();
    // Seed the cache
    await cache.get("settings", () => Promise.resolve({ theme: "dark" }), 10_000);
