...
class RateLimiter {
     ...
     */
    constructor(limitCount, windowMs) {
...
async function exampleSMSLimiter() {
    console.log("\n=== Example 1: SMS Rate Limiter (2 req / sec) ===");
    const smsLimiter = new RateLimiter(2, 1000);
    ...
    const results = await Promise.all(phones.map((phone) => smsLimiter
        .add(() => sendSMS(phone))
        .then((res) => {
...
async function exampleApiThrottle() {
    console.log("\n=== Example 2: API Call Throttle (3 req / 2 sec) ===");
    const apiLimiter = new RateLimiter(3, 2000);
    let callCount = 0;
    ...
    const ids = [1, 2, 3, 4, 5];
    await Promise.allSettled(ids.map((id) => apiLimiter
        .add(() => callApi(id))
        .then((r) => console.log(`  ✅ id=${id}:`, r))
        .catch((e) => console.log(`  ❌ id=${id}: ${e.message}`))));
}
// ── Example 3: Queue clear / drain ───────────────────────────
async function exampleQueueClear() {
    console.log("\n=== Example 3: Queue Clear ===");
    const limiter = new RateLimiter(1, 2000); // very slow: 1 req / 2 sec
    // Add 5 tasks immediately — the first fires right away, rest queue up.
    const promises = Array.from({ length: 5 }, (_, i) => limiter
